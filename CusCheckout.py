...
class Ui_frm_cus_checkout(object):
    ...
    def useCoupon(self):
        self.code = self.txt_coupon.text().upper().strip()
        with GetDatabase() as conn:
            db = conn.get_database('ucwb')
            condition = {'code': self.code}
            found = db.coupons.count_documents(condition)
            if found:
                cursor = db.coupons.find(condition)
                self.discount = cursor[0]['value']
            else:
                self.discount = 0
            self.addToTable(self.cart)
